...
def induction_vol_integral(clus_b2, MIE_diver_B2, MIE_compres_B2, MIE_stretch_B2, MIE_advec_B2, MIE_drag_B2, MIE_total_B2,
                            cinetic_energy_density, clus_cr0amr, clus_solapst, grid_zeta,
                            grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size, nmax,
                            coords, rad, a0, level, clus_kp, grid_irr, units =1, verbose=False):
    '''
    Computes the volume integral of the magnetic energy density and its components, as well as the induced magnetic energy.
    This is done according to the derived equation and compared to the actual magnetic energy integrated along the studied volume. The cinetic energy
    density is also computed.
    
    Args:
        - clus_b2: magnetic energy density in the cluster
        - MIE_diver_B2, MIE_compres_B2, MIE_stretch_B2, MIE_advec_B2, MIE_drag_B2, MIE_total_B2: components of the magnetic induction energy
        - cinetic_energy_density: kinetic energy density
    ...
    Returns:
        ...
        - int_cinetic_energy: volume integral of the kinetic energy density
    ...
    int_b2 = utils.vol_integral(clus_b2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_diver_B2 = utils.vol_integral(MIE_diver_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_compres_B2 = utils.vol_integral(MIE_compres_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_stretch_B2 = utils.vol_integral(MIE_stretch_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_advec_B2 = utils.vol_integral(MIE_advec_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_drag_B2 = utils.vol_integral(MIE_drag_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_MIE_total_B2 = utils.vol_integral(MIE_total_B2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    int_cinetic_energy = utils.vol_integral(cinetic_energy_density, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
                            grid_patchnx, grid_patchny, grid_patchnz, size[0], nmax, coords, rad, max_refined_level=level, kept_patches=clus_kp)
    volume = utils.vol_integral(clus_b2, units, a0, grid_zeta, clus_cr0amr, clus_solapst, grid_npatch, grid_patchrx, grid_patchry, grid_patchrz,
...
def induction_radial_profiles(clus_b2, MIE_diver_B2, MIE_compres_B2, MIE_stretch_B2, MIE_advec_B2, MIE_drag_B2,
                            MIE_total_B2, clus_cr0amr, clus_solapst, grid_npatch,
                            grid_patchrx, grid_patchry, grid_patchrz, grid_patchnx, grid_patchny,
                            grid_patchnz, clus_kp, grid_irr, it, sims, size, nmax, coords, rmin, rad, nbins=25, logbins=True,
                            level=100, verbose=False):
    '''
    ...
    This is done according to the derived equation and compared to the actual magnetic energy integrated along the studied profile. The cinetic energy
    ...
    clus_b2_clean = utils.clean_field(clus_b2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del clus_B2
    gc.collect()
    profile_bin_centers, clus_B2_profile = utils.radial_profile_vw(field=clus_b2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
    ...
    MIE_diver_B2_clean = utils.clean_field(MIE_diver_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_diver_B2
    gc.collect()
    _, MIE_diver_B2_profile = utils.radial_profile_vw(field=MIE_diver_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Divergence profile done')
    MIE_compres_B2_clean = utils.clean_field(MIE_compres_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_compres_B2
    gc.collect()
    _, MIE_compres_B2_profile = utils.radial_profile_vw(field=MIE_compres_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Compressive profile done')
    MIE_stretch_B2_clean = utils.clean_field(MIE_stretch_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_stretch_B2
    gc.collect()
    _, MIE_stretch_B2_profile = utils.radial_profile_vw(field=MIE_stretch_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Stretching profile done')
    MIE_advec_B2_clean = utils.clean_field(MIE_advec_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_advec_B2
    gc.collect()
    _, MIE_advec_B2_profile = utils.radial_profile_vw(field=MIE_advec_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Advection profile done')
    MIE_drag_B2_clean = utils.clean_field(MIE_drag_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_drag_B2
    gc.collect()
    _, MIE_drag_B2_profile = utils.radial_profile_vw(field=MIE_drag_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Drag profile done')
    MIE_total_B2_clean = utils.clean_field(MIE_total_B2, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del MIE_total_B2
    gc.collect()
    _, MIE_total_B2_profile = utils.radial_profile_vw(field=MIE_total_B2_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Total profile done')
    cinetic_energy_clean = utils.clean_field(cinetic_energy_density, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del cinetic_energy_density
    gc.collect()
    _, cinetic_energy_profile = utils.radial_profile_vw(field=cinetic_energy_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    print(f'Snap {it} in {sims}: Cinetic profile done')
    clus_rho_rho_b_clean = utils.clean_field(clus_rho_rho_b, clus_cr0amr, clus_solapst, grid_npatch, up_to_level=level)
    del clus_rho_rho_b
    gc.collect()
    _, clus_rho_rho_b_profile = utils.radial_profile_vw(field=clus_rho_rho_b_clean, clusrx=coords[0], clusry=coords[1], clusrz=coords[2], rmin=rmin, rmax=rad,
                                            nbins=nbins, logbins=logbins, cellsrx=X, cellsry=Y, cellsrz=Z, cr0amr=clus_cr0amr,
                                            solapst=clus_solapst, npatch=grid_npatch, size=size[0], nmax=nmax, up_to_level=level)
    ...
    return (
        ...
        cinetic_energy_profile,
        clus_rho_rho_b_profile
    )
    
    
def induction_section_projection(field, nmax, size, Box,
    ...
    '''
    Cleans and computes the uniform section of the given field for the given AMR grid for its further projection.
    
    Args:
        - field: field to be cleaned and projected
    ...
    if verbose == True:
        print('Uniform field projection done.')
